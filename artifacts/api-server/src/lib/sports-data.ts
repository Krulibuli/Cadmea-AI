...
export interface SportsFacility {
  ...
  annualVisits: number;
  /** External rating imported from public listings, when available */
  rating?: number;
  /** Review count used as a proxy traffic signal for imported public listings */
  reviewCount?: number;
  /** 0-100 traffic signal derived from review volume or modeled occupancy */
  trafficScore?: number;
  /** Human-readable source for traffic data */
...
export function getOccupancy(id: string): OccupancyResponse | null {
  ...
  return {
    ...
    hour: vilniusHour,
    notes: f.reviewCount
      ? "Modelled from imported public listing review volume and typical hourly gym traffic patterns."
      : isOperational
...
interface ImportedSportLocation {
  district: string;
  type: string;
  name: string;
  address: string;
...
const IMPORTED_SPORT_LOCATIONS = sportLocationsJson as ImportedSportLocation[];
...
function importedType(type: string): { type: FacilityType; disciplines: Discipline[] } {
  const normalized = type.toLowerCase();
  if (normalized.includes("tennis")) return { type: "tennis", disciplines: ["tennis", "fitness"] };
  if (normalized.includes("ice")) return { type: "ice_rink", disciplines: ["ice_skating", "fitness"] };
  if (normalized.includes("stadium")) return { type: "stadium", disciplines: ["football", "athletics"] };
  if (normalized.includes("pitch")) return { type: "pitch", disciplines: ["football", "outdoor"] };
  return { type: "fitness_centre", disciplines: ["fitness", "general"] };
}

function trafficScoreFromReviews(reviews: number) {
  if (!Number.isFinite(reviews) || reviews <= 0) return 8;
  const maxReviews = 18045;
  return Math.max(8, Math.min(100, Math.round(Math.sqrt(reviews / maxReviews) * 100)));
}

function buildImportedLocations(): SportsFacility[] {
  return IMPORTED_SPORT_LOCATIONS.map((row, index) => {
    const classified = importedType(row.type);
    const district = row.district || nearestDistrict(row.lat, row.lng);
    const id = `csv-${slug(`${row.name}-${row.address}-${index}`)}`;
    const tags = tagAgeAndAccess(classified.type, classified.disciplines);
    const trafficScore = trafficScoreFromReviews(row.reviews);
    const kpis = buildKpis(id, classified.type, "operational");
    return {
      id,
      name: row.name,
      ...
      status: "operational",
      ...kpis,
      capacity: Math.max(60, Math.min(1200, Math.round(80 + row.reviews * 0.42))),
      utilization: trafficScore,
      annualVisits: Math.max(kpis.annualVisits, Math.round(row.reviews * 95)),
      rating: row.rating,
      reviewCount: row.reviews,
      trafficScore,
      trafficSource: "Imported listing review volume",
    };
  });
}
...
function buildAll(): SportsFacility[] {
  ...
  out.push(...buildImportedLocations());
  return out;
}
