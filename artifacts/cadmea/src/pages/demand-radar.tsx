...
import { Radar, TrendingUp, AlertTriangle, MapPin, Sparkles, Info, Hash, Grid3x3, Rocket, Flame } from "lucide-react";
...
import { DISCIPLINE_LABEL, useSportsHeatmap, useSportsMeta, type Discipline, type SportsHeatmapCell } from "@/lib/sports-api";

const VILNIUS_CENTER: [number, number] = [54.6872, 25.2797];
const VILNIUS_BOUNDS: [[number, number], [number, number]] = [[54.52, 24.82], [54.9, 25.68]];
...
export default function DemandRadarPage() {
  ...
  const r = report.data;
  const heatCells = heatmap.data?.cells ?? [];
  const topHeatCells = heatCells.slice(0, 5);
            ...
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                <h2 className="flex items-center gap-2 text-lg font-extrabold text-foreground">
                  ...
                  {language === "lt" ? "Sporto šakų heatmapas" : "Sport heatmap"}
                </h2>
                <p className="mt-1 max-w-3xl text-sm text-muted-foreground">
                  {language === "lt"
                    ? "Pasirinkite sporto šaką ir matykite, kuriuose rajonuose poreikis didžiausias. Skaičiavimas naudoja Active Vilnius Demand Radar / Strava proxy signalą, infrastruktūros aprėptį, objektų tankį ir užimtumą."
                    : "Choose a sport and see where demand is highest. The score uses Demand Radar / Strava proxy signals, coverage, density, and utilization."}
                </p>
              </div>
              <div className="w-full sm:w-72">
                <Select value={discipline} onValueChange={setDiscipline}>
                  <SelectTrigger className="bg-card">
                    <SelectValue placeholder={language === "lt" ? "Sporto šaka" : "Sport"} />
                  </SelectTrigger>
                  <SelectContent>
          ...
          <div className="grid lg:grid-cols-[minmax(0,1fr)_320px]">
            <div className="relative h-[56vh] min-h-[460px]">
                ...
                {heatCells.map((cell) => {
                  ...
                  return (
                    <Circle
                      ...
                      pathOptions={{ color, fillColor: color, fillOpacity: 0.22, opacity: 0.85, weight: 2 }}
                    >
                      <Popup>
              ...
              <div className="absolute bottom-3 left-3 z-[500] rounded-md border border-border bg-card/95 p-2 text-[11px] shadow-lg backdrop-blur">
                <div className="mb-1 font-bold text-foreground">{language === "lt" ? "Poreikio indeksas" : "Demand index"}</div>
                <div className="flex flex-wrap items-center gap-2">
                  {[
                    { label: language === "lt" ? "Žemas" : "Low", color: "#16A34A" },
                    { label: language === "lt" ? "Vid." : "Med.", color: "#1F6F8B" },
                    { label: language === "lt" ? "Aukštas" : "High", color: "#F59E0B" },
                    { label: language === "lt" ? "Kritinis" : "Critical", color: "#C8102E" },
                  ].map((item) => (
            ...
            <aside className="border-t border-border p-4 lg:border-l lg:border-t-0">
              <h3 className="mb-3 text-sm font-extrabold text-foreground">
                {language === "lt" ? "Prioritetiniai rajonai" : "Priority districts"}
              </h3>
              <div className="space-y-2">
                {topHeatCells.map((cell) => (
                  <div key={`${cell.district}-heat-rank`} className="rounded-md border border-border bg-muted/25 p-3">
