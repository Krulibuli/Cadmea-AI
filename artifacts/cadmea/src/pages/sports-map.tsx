...
import {
  ...
  FACILITY_TYPE_LABEL, DISCIPLINE_LABEL, AGE_GROUP_LABEL, formatInt,
...
function occupancyLevel(value: number) {
  if (value >= 70) return { label: "Didelis", color: "#DC2626" };
  if (value >= 40) return { label: "Vidutinis", color: "#EAB308" };
  return { label: "Mažas", color: "#16A34A" };
}
          ...
          <Card className="lg:col-span-2 overflow-hidden">
            <div className="h-[60vh] min-h-[420px]">
              <MapContainer center={center} zoom={11} className="h-full w-full" key={`${center[0]}-${center[1]}`}>
                ...
                {list.map((f) => (
                  (() => {
                    const occupancy = f.currentOccupancy ?? f.utilization;
                    ...
                    return (
                      <CircleMarker
                        ...
                        pathOptions={{
                          ...
                          fillOpacity: f.status === "operational" ? 0.78 : 0.35,
                          opacity: 0.95,
                          weight: 1.5,
                          ...
                          <div className="space-y-1">
                            ...
                            {typeof f.reviewCount === "number" && (
                              <p className="text-xs text-muted-foreground">
                                Traffic proxy: {f.trafficScore ?? occupancy}/100 · {formatInt(f.reviewCount)} reviews
              ...
              <div className="absolute bottom-3 left-3 z-[500] rounded-md border border-border bg-card/95 p-2 text-[11px] shadow-lg backdrop-blur">
                <div className="mb-1 font-bold text-foreground">
                  {language === "lt" ? "Objekto užimtumas" : "Facility occupancy"}
                </div>
                <div className="flex flex-wrap items-center gap-2">
                  {[
                    { label: language === "lt" ? "Mažas" : "Low", color: "#16A34A" },
                    { label: language === "lt" ? "Vidutinis" : "Medium", color: "#EAB308" },
                    { label: language === "lt" ? "Didelis" : "High", color: "#DC2626" },
                  ].map((item) => (
                ...
                <p className="mt-1 text-[10px] text-muted-foreground">
                  {language === "lt" ? "Atnaujinama kas valandą" : "Refreshes hourly"}
                </p>
              </div>
            </div>
          </Card>
                      ...
                      <div className="flex items-start justify-between gap-2">
                        <div className="min-w-0 flex-1">
                          ...
                          {typeof f.reviewCount === "number" && (
                            <p className="mt-1 text-[11px] font-semibold text-muted-foreground">
                              Traffic {f.trafficScore ?? f.currentOccupancy ?? f.utilization}/100 · {formatInt(f.reviewCount)} reviews
                            </p>
                          )}
                        </div>
                        {f.status === "operational" ? (
                          <Badge
                            ...
                            className="text-[10px]"
                            style={{
                              borderColor: occupancyLevel(f.currentOccupancy ?? f.utilization).color,
                              color: occupancyLevel(f.currentOccupancy ?? f.utilization).color,
                            }}
                          >
                            {f.currentOccupancy ?? f.utilization}%
                          </Badge>
                        ) : (
